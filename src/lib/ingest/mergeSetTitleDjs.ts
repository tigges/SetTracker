/**
 * Fold set-title / episode Dj rows onto real artists.
 *
 * Examples:
...
import { performingCreditFromTitle, tidyPerformingCredit } from "./artists";
...
export function resolveCanonicalFromSetTitleDj(
  ...
  if (MEGA_MIX.test(name) || /mega-mix/.test(slug) || /night\s*owl/i.test(name)) {
    if (
  ...
  if (PODCAST_EP.test(name) && /\bdom\s*dolla\b/i.test(name)) {
...
async function ensureDj(
  ...
  if (existing) {
    ...
    if (
      ...
      /at |\/\/|mix|live/i.test(existing.name)
    ) {
...
/**
 * Scan Dj rows that look like set titles and fold them onto real artists.
 */
export async function mergeSetTitleDjs(
  ...
  for (const dj of candidates) {
    ...
    scanned += 1;
    const target = resolveCanonicalFromSetTitleDj(dj.name, dj.slug);
