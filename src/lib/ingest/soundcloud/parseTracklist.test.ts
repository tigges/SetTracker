...
assert.equal(clapcast576[5]!.artistName, "Adam Ten & Darco");
assert.equal(clapcast576[5]!.trackTitle, "Limousine (Original Mix) [ID]");
...
assert.ok((clapcast576[0]!.timestamp ?? 0) < (clapcast576[13]!.timestamp ?? 0));
