...
assert.equal(isLeftoverHostName("Chris Lake Full"), true);
assert.equal(isJunkArtistName("Chris Lake Full"), true);
assert.equal(isLeftoverHostName("James Hype Official"), true);
assert.equal(isLeftoverHostName("Charlotte de Witte"), false);
assert.equal(sanitizeArtistName("Chris Lake Full"), "Chris Lake");
assert.equal(sanitizeArtistName("James Hype Official"), "James Hype");
assert.equal(isJunkArtistName("Push only"), true);
assert.equal(isJunkArtistName("Bonobo Solo"), true);
assert.equal(isJunkArtistName("Mike Williams Throwback"), true);
assert.equal(isJunkArtistName("Enrico Sangiuliano Lockdown"), true);
assert.equal(isJunkArtistName("Yotto's Odd World"), true);
assert.equal(isJunkArtistName("Push"), false);
assert.equal(isJunkArtistName("Bonobo"), false);
assert.equal(isJunkArtistName("Lockdown"), false);
assert.equal(sanitizeArtistName("Push only"), "Push");
assert.equal(sanitizeArtistName("Bonobo Solo"), "Bonobo");
assert.equal(sanitizeArtistName("Yotto's Odd World"), "Yotto");
